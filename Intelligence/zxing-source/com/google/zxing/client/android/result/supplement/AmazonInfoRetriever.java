...
final class AmazonInfoRetriever extends SupplementalInfoRetriever {
	...
	private boolean doRetrieveForCountry(String theCountry) throws IOException {
		...
		CharSequence contents = HttpHelper.downloadViaHttp(
				"https://bsplus.srowen.com/ss?c=" + theCountry + "&t=" + type
						+ "&i=" + productID, HttpHelper.ContentType.XML);
		...
		append(productID, "Amazon " + theCountry,
				newTexts.toArray(new String[newTexts.size()]), detailPageURL);
		return true;
	}
	...
	private static XmlPullParser buildParser(CharSequence contents)
